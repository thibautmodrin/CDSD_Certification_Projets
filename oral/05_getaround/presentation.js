...
async function main() {
  ...
  const demos = [
    { n: "01", t: "API /docs", d: "getaround-api-sit1.onrender.com" },
    { n: "02", t: "Dashboard", d: "getaround-dashboard-yims.onrender.com" },
  ...
  const lims = [
    { now: "MLflow absent ici (pas Docker HF)", next: "→ citer HPP (MLflow Space)" },
    { now: "Render Free : cold start ~30–60 s", next: "→ ouvrir URLs avant l'oral" },
    { now: "Pas de CI/CD", next: "→ GitHub Actions" },
    { now: "Monitoring / drift absent", next: "→ logs + alertes" },
    { now: "Modèle Ridge simple", next: "→ RF / tuning si besoin" },
  ];
  lims.forEach((l, i) => {
    s.addText(l.now, {
  ...
  const outPath = path.join(__dirname, "Presentation_Bloc5_Getaround.pptx");
